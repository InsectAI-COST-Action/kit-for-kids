...
def check_data_safety_contract() -> None:
    ...
    require("def prepare_card(" in text("tools/prepare_sd.py"), "SD preparation must expose a reusable prepare_card function")
    require('CARD_VOLUME_LABEL = "INSECT-AI"' in text("tools/prepare_sd.py") and "set_volume_label(destination, CARD_VOLUME_LABEL)" in text("tools/prepare_sd.py"), "SD preparation must name the drive to match what the dashboard tells users to pick")
    setup = text("tools/setup_device.py")
    require("from prepare_sd import ROOT, prepare_card" in setup and "\"-t\", \"upload\"" in setup and "xiao_esp32s3" in setup and "--flash-only" in setup and "--skip-flash" in setup, "One-command setup must wrap both the firmware upload and prepare_card")
...
def check_dashboard_contract() -> None:
    ...
    i18n = text("dashboard/i18n.js")
    require("const LOCALES = [" in i18n and "translations = {" in i18n and "en: {" in i18n and "es: {" in i18n and "setLocale" in i18n and "buildLocalePicker" in i18n, "Dashboard i18n module must offer a locale registry, not a hardcoded English/Spanish toggle")
    ...
    require("Pause search" in html and "Stop search" in html and "possible insect" in i18n and "createImageBitmap" in analysis, "Browser analysis must use safe selected-file images and stay controllable")
    ...
    require("Show all" in html and "Escape" in javascript, "Dashboard must support expansion and modal close")
    ...
    require(html.count('data-i18n="card.loadButton"') >= 2 and "Load images and AI" not in html, "Both card-loading buttons must carry the same name")
    ...
    require("analysis-session" in html and "selectedAnalysisEntries" in analysis and "refreshAnalysisSessions" in analysis and "Newest session" in i18n, "AI analysis must let users select one available camera session")
    ...
    require("settings-interval" in html and "settings-quality" in html and "settings-duration" in html and "settings-motion-trigger" in html and all(option in html for option in ("1 image every 1 second", "1 image every 2 seconds", "1 image every 30 seconds", "1 image every 1 minute", "High quality", "Low quality", "1 minute", "5 minutes", "30 minutes", "1 hour", "Infinite - until switched off")) and "capture_interval_ms" in settings and "motion_trigger_enabled" in settings and "motion_threshold" in settings and "applyCurrentToControls" in settings and "choices below match it" in i18n and "window.confirm" in settings and "qxga_q12_1fps" in settings, "Camera settings must offer every validated interval, quality, duration, and motion choice and display the active card configuration")
    ...
    require('verb == "PROBE"' in dev_bridge and "camera->begin(" in dev_bridge and "context.camera == nullptr" in dev_bridge, "PROBE must check the camera independently of storage, so a disconnected expansion board is distinguishable from a bad SD card")
    require("CameraService* camera" in text("include/dev_bridge.h"), "DevBridgeContext must expose the camera for PROBE")
    require("dev_context.camera = &camera" in text("src/main.cpp"), "main.cpp must wire the real camera into the dev-bridge context")
    require("probe" in text("tools/dev_bridge_client.py"), "The dev-bridge client must expose the PROBE command")

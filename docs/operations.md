...
Run `py tools\setup_device.py` on a development computer with the board plugged
in and a formatted FAT32 card in a reader. It flashes the firmware, then runs the
card preparation below. Use `--flash-only` or `--skip-flash --card <root>` to do
one half; `--port`, `--card` and `--yes` make it non-interactive. See the
...
4. To try experimental browser AI in Chrome or Edge: as of 18 September 2026 both FlatBug and AntAI - Test ship automatically with `py tools\prepare_sd.py <card-root>` (Part 3); for an older card prepared before that date, add them with `py tools\install_ai_pack.py <card-root>`. Select **Find insects with AI**, choose one option: **AntAI - Test** (ant-only), **FlatBug - Quick look** (faster) or **FlatBug - Look closely** (slower, 12 tiles). Select **Load camera card**, choose the clearly labelled **INSECT-AI** drive, then choose the desired saved session and press **Start looking**. The dashboard analyses only that session; it never mixes image records from separate experiments. The same selected card can be reused by the movie maker until the tab closes. Pictures are analysed locally, one at a time, with live possible-insect clues and pause/stop controls. These are predictions, not validated identifications, and nothing is uploaded or written back to the card.
...
7. For a safe rehearsal demonstration (real reference ant photos, fabricated run/timing data), run `py tools\install_dashboard_demo.py <card-root>`, then open `<card-root>\demo\demo.html` - it opens already loaded (images and AI models are embedded at install time), no folder to choose, so you can go straight to **Start looking**. This is visibly separate from real captures. The demo also includes its own practice copies of **Make my insect movie** and **Camera settings** for risk-free rehearsal; the settings tool writes only to the demo folder's own `config.json`, never to a real camera card.

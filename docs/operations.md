...
5. Select an image to open it in the page modal; use the `x` control, Escape, or the backdrop to close it.
6. For a safe, colourful synthetic demonstration, run `py tools\install_dashboard_demo.py <card-root>`, open `<card-root>\demo\demo.html`, choose the `demo` folder itself, then select **Start looking**. This is visibly separate from real captures.
7. Use the computer's file manager, not the dashboard, to delete data when starting a new experiment. Do not delete files during collection.
8. Preserve `/raw/captures.csv` when analysis matters: it is the authoritative record.
